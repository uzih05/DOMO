...
import { updateRecruitment } from '@/src/models/api';
...
export const EditRecruitmentModal: React.FC<EditRecruitmentModalProps> = ({
  recruitment,
  onClose,
  onUpdated,
}) => {
  ...
  const [category, setCategory] = useState<RecruitmentCategory>(recruitment.category);
  const [region, setRegion] = useState<JeonbukRegion>(recruitment.region);
  const [techStacks, setTechStacks] = useState<string[]>(
    recruitment.tech_stacks.map(ts => ts.name)
  );
  const [customStack, setCustomStack] = useState('');
  const [positionSlots, setPositionSlots] = useState<Omit<PositionSlot, 'filled'>[]>(
    recruitment.position_slots.map(slot => ({
      position: slot.position,
      total: slot.total,
    }))
  );
  const [startDate, setStartDate] = useState(recruitment.start_date?.split('T')[0] || '');
  const [endDate, setEndDate] = useState(recruitment.end_date?.split('T')[0] || '');
  const [deadline, setDeadline] = useState(recruitment.deadline?.split('T')[0] || '');
  const [contactInfo, setContactInfo] = useState(recruitment.contact_info || '');
  const [referenceUrl, setReferenceUrl] = useState(recruitment.reference_url || '');
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const data: RecruitmentUpdateRequest = {
        title: title.trim(),
      ...
      await updateRecruitment(recruitment.id, data);
      onUpdated();
    } catch (err) {
      setError(err instanceof Error ? err.message : '모집글 수정에 실패했습니다.');
    } finally {
      setLoading(false);
    }
  };
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm">
      <div className="glass-panel rounded-2xl w-full max-w-2xl max-h-[90vh] mx-4 overflow-hidden animate-fade-in">
        <div className="flex items-center justify-between px-6 py-4 border-b border-[var(--glass-border)]">
          <h2 className="text-xl font-bold text-[var(--text-primary)]">모집글 수정</h2>
          <button
            onClick={onClose}
