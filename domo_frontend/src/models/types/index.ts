...
// ============================================
// 11. Domo-Match (프로젝트/스터디 매칭)
// ============================================

/**
 * 전북 지역 코드
 */
export type JeonbukRegion =
  | 'all'        // 전북 전체
  | 'jeonju'     // 전주
  | 'iksan'      // 익산
  | 'gunsan'     // 군산
...
export const REGION_LABELS: Record<JeonbukRegion, string> = {
  all: '전북 전체',
  jeonju: '전주',
  iksan: '익산',
  gunsan: '군산',
...
export type RecruitmentCategory =
  ...
  | 'hackathon'       // 교내/교외 공모전(해커톤)
  | 'study'           // 스터디(언어/기술)
  | 'mentoring';      // 멘토링 요청

export const CATEGORY_LABELS: Record<RecruitmentCategory, string> = {
  side_project: '사이드 프로젝트',
  hackathon: '공모전/해커톤',
  study: '스터디',
  mentoring: '멘토링',
...
export const POSITION_LABELS: Record<RecruitmentPosition, string> = {
  frontend: '프론트엔드',
  backend: '백엔드',
  fullstack: '풀스택',
  designer: '디자이너',
 ...
 * 모집 상태
 */
export type RecruitmentStatus = 'recruiting' | 'closing_soon' | 'closed';
...
export interface RecruitmentCreateRequest {
  ...
  category: RecruitmentCategory;
  region: JeonbukRegion;
  tech_stacks: string[];       // 기술 스택 이름 배열
  position_slots: Omit<PositionSlot, 'filled'>[];
  start_date?: string;
  end_date?: string;
  deadline?: string;
  contact_info?: string;
...
export interface Application {
  ...
  portfolio_url?: string;      // GitHub, 포트폴리오 링크
...
export interface MentorBadge {
  id: number;
  user_id: number;
  badge_type: 'professional' | 'senior_student';
  description?: string;
  granted_at: string;
}
